 ...
 */
@Stateless
@Remote({ClimateSpatialExtension.class})
public class ClimateSpatialExtensionBean implements ClimateSpatialExtension {

	private static Logger LOG = LogManager.getLogger(Loggers.COMPONENT_LOG, ClimateSpatialExtensionBean.class);
	
	private static String ZIP_QUERY = "select lat, lon from ZipCodes where zip_code = ?";
	
	private static String STATION_QUERY = "select s.station_id FROM absolute_location al inner join location_station_link lsl on al.location_id = " +
			"lsl.location_id inner join station s on lsl.station_id = s.station_id WHERE 1=1 AND 3963.191 * ACOS( (SIN(PI()* LAT /180)*SIN(PI() * latitude/180)) + " +
	...
	private DataSource firm_source = DataSourceInjector.injectDataSource(DataSourceTypes.SYSTEM);
	
	@EJB(name="SpatialQuery", beanInterface=SpatialQuery.class)
	private SpatialQuery spatialQuery;
	
	@EJB(name="ClimateMetaDataQuery", beanInterface=ClimateMetaDataQuery.class)
	private ClimateMetaDataQuery metaQuery;
	
	@WebMethod
	public List<String> getStationsByZipCode(String zipCode, int distance) throws RemoteException {
		...
		try { 
			...
			if ( zip_code_result.next() ) { 
				...
				String station_query_string = STATION_QUERY.replaceAll("LAT", lat);
				station_query_string = station_query_string.replaceAll("LON", lon);
				
				station_query = station_conn.prepareStatement(station_query_string);
	...
	@WebMethod
	public USCounty getCounty(String station) throws RemoteException {
		MetaDataCollection<StationMetaDataType> counties;
		ArrayList<String> stations = new ArrayList<String>();
		stations.add(station);
		try {
			counties = metaQuery.getAllMetaData(stations, CalendarPeriod.WEEKLY);
		} catch ( InvalidArgumentException iae ) { 
			LOG.error("bad argument to the meta data query", iae);
			RemoteException re = new RemoteException();
			re.initCause(iae);
		...
		String county_name = (String)counties.getStationMetaData(station).get(StationMetaDataType.COUNTY);
		String state_name = (String)counties.getStationMetaData(station).get(StationMetaDataType.STATE);
		try {
			return spatialQuery.searchCountiesByState(county_name, USState.valueOf(state_name)).iterator().next();
		} catch ( NoSuchElementException nse ) { 
			LOG.warn("No county could be found for: "+county_name+" in state "+state_name);
			return null;
		}
	}
	...
	@WebMethod
	public List<String> getStationsForState(USState state) throws RemoteException {
		...
		try { 
			...
			stmt = conn.prepareStatement("select station_id from station where state = ? order by station_name");
	...
	@WebMethod
	public List<String> getStationsForGeographicRegion(USRegion region) throws RemoteException {		
		Connection conn = null;
		ArrayList<String> station_ids = new ArrayList<String>();
		PreparedStatement stmt = null;
		ResultSet station_query = null;
		try { 
			...
			stmt = conn.prepareStatement("select station_id from station where state = ? order by state, station_name");
	...
	@WebMethod
	public List<String> getStationsForDefinedRegion(BoundingBox region) throws RemoteException {
		Connection conn = null;
		ArrayList<String> station_ids = new ArrayList<String>();
		PreparedStatement stmt = null;
		ResultSet station_query = null;
		try { 
			...
			stmt = conn.prepareStatement("select s.station_id from station s inner join location_station_link lsl on " +
					"s.station_id = lsl.station_id inner join absolute_location al on lsl.location_id = al.location_id where al.longitude >= ? and " +
					"al.longitude <= ? and al.latitude >= ? and al.latitude <= ? order by s.state, s.station_name");
	...
	public List<String> getStationsFromPoint(float lat, float lon, int distance)
		...
		try { 
			...
			RemoteException re = new RemoteException("could not get a connection");
			re.initCause(e);
			throw re;
		}
		
		try {
			PreparedStatement stmt = conn.prepareStatement("SELECT location_station_link.station_id, ( 3959 * acos( cos( radians(?) ) * cos( radians( latitude ) ) * " +
					"cos( radians( longitude ) - radians(?) ) + sin( radians(?) ) * sin( radians( latitude ) ) ) ) AS distance FROM " +
					"absolute_location inner join location_station_link on location_station_link.location_id = " +
					"absolute_location.location_id HAVING distance < ?;");
