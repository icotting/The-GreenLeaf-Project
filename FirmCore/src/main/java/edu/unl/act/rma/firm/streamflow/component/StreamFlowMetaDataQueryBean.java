...
@Stateless
@Remote( { StreamFlowMetaDataQuery.class })
public class StreamFlowMetaDataQueryBean implements StreamFlowMetaDataQuery {
	private static Logger LOG = LogManager.getLogger(Loggers.COMPONENT_LOG,
			StreamFlowMetaDataQueryBean.class);
	...
	@EJB(name = "StreamFlowSpatialExtension", beanInterface = StreamFlowSpatialExtension.class)
	private StreamFlowSpatialExtension spatialExtension;
	
	@EJB(name="StreamFlowDataQuery", beanInterface=StreamFlowDataQuery.class)
	private StreamFlowDataQuery dataQuery;

	@WebMethod
	public TemporalPeriod getLongestPeriod(List<String> stations,
			CalendarPeriod period, DataType type) throws RemoteException,
		...
		DateTime end = new DateTime(new DateTime(1800, 1, 1, 0, 0, 0, 0,
				GregorianChronology.getInstance()).getMillis());

		Connection conn = null;
		try {
			conn = source.getConnection();
			String sql = "SELECT MIN(start_date), MAX(end_date) FROM "
					+ period.name()
					+ ", variable "
					+ "WHERE variable.station_id = ? AND "
					+ period.name()
					+ ".variable_id = variable.variable_id AND variable.variable_name = ? ";
			...
			for (String station : stations) {
				...
				rs = ps.executeQuery();
				if (rs.next()) {
					if (rs.getDate(1).before(begin.toDate())) {
						begin = new DateTime(rs.getDate(1));
					}
					if (rs.getDate(2).after(end.toDate())) {
						end = new DateTime(rs.getDate(2));
					}
				}
			}

		} catch (SQLException sqe) {
			LOG.error("error querying station start/end dates", sqe);
		} finally {
			if (null != conn) {
				try {
					conn.close();
	...
	@WebMethod
	public MetaDataCollection<StationMetaDataType> findStations(
			StationSearchTerms terms) throws RemoteException {
		Connection conn = null;
		...
		try {
			...
			if (terms.getStationID() != null) {
				stmt = conn
						.prepareStatement("SELECT station_id FROM station WHERE station_id = ?");
				...
				station_ids.addAll(spatialExtension.getStationsByZipCode(terms
						.getZipCode(), 10));
			} else if (terms.getCounty() != null) {
				stmt = conn
						.prepareStatement("SELECT station_id FROM station WHERE UPPER(county) LIKE ?"
								+ (terms.getState() == null ? ""
										: " AND STATE = ?"));
				...
				stmt = conn
						.prepareStatement("SELECT latitude, longitude FROM station WHERE UPPER(station_name) LIKE ?"
								+ (terms.getState() == null ? ""
										: " AND STATE = ?"));
				city_stmt = conn
						.prepareStatement("SELECT station_id FROM station WHERE latitude > ? AND "
				...
				while (rs1.next()) {
					...
					// now for each station found by name, search in the same
					// area (by lat/long)
				...
				for (String id : spatialExtension.getStationsForState(terms
						.getState())) {
					if (!station_ids.contains(id)) {
						station_ids.add(id);
					}
			...
			if (terms.getStationName() != null) {
				stmt = conn
						.prepareStatement("SELECT station_id FROM station WHERE UPPER(station_name) LIKE ?");
		...
		try {
			mdc = getMetaData(station_ids, StationMetaDataType.STATION_NAME,
					CalendarPeriod.DAILY);
			mdc.importMap(StationMetaDataType.LATITUDE, getMetaData(
					station_ids, StationMetaDataType.LATITUDE,
					CalendarPeriod.DAILY).extractType(
					StationMetaDataType.LATITUDE));
			mdc.importMap(StationMetaDataType.LONGITUDE, getMetaData(
					station_ids, StationMetaDataType.LONGITUDE,
					CalendarPeriod.DAILY).extractType(
					StationMetaDataType.LONGITUDE));
			mdc.importMap(StationMetaDataType.COUNTY, getMetaData(station_ids,
					StationMetaDataType.COUNTY, CalendarPeriod.DAILY)
					.extractType(StationMetaDataType.COUNTY));
			mdc.importMap(StationMetaDataType.STATE, getMetaData(station_ids,
					StationMetaDataType.STATE, CalendarPeriod.DAILY)
					.extractType(StationMetaDataType.STATE));
			mdc.importMap(StationMetaDataType.START_DATE, getMetaData(
					station_ids, StationMetaDataType.START_DATE,
					CalendarPeriod.DAILY).extractType(
					StationMetaDataType.START_DATE));
			mdc.importMap(StationMetaDataType.END_DATE, getMetaData(
					station_ids, StationMetaDataType.END_DATE,
					CalendarPeriod.DAILY).extractType(
					StationMetaDataType.END_DATE));
		} catch (InvalidArgumentException iae) {
			LOG.error("error collecting metadata", iae);
			throw new RemoteException(
					"unable to query metadata for station criteria");
	...
	private HashMap<StationMetaDataType, Object> getMetaDataFromQuery(
			ResultSet meta_query_result, CalendarPeriod period)
		...
		while (meta_query_result.next()) {
			for (int i = 1; i <= field_count; i++) {
				current_type = StationMetaDataType.valueOf(res_meta
						.getColumnName(i).toUpperCase());
				station_meta_data.put(current_type, getTypedValue(
						meta_query_result, current_type, i, period));
			}
		}
	...
	public MetaDataCollection<StationMetaDataType> getMetaData(List<String> stations, StationMetaDataType field, CalendarPeriod period) throws InvalidArgumentException, RemoteException {
		...
		try { 
			...
			stmt = conn.prepareStatement("select "+field.getFieldName()+" from station where  station.station_id = ?");
	...
	public MetaDataCollection<StationMetaDataType> getAllMetaData(List<String> stations, CalendarPeriod period) throws InvalidArgumentException, RemoteException {		
		...
		try { 
			...
			stmt = conn.prepareStatement("select "+types+" from station where station.station_id = ?");
	...
	throws RemoteException {		
		...
		for ( VariableFilter filter : filters ) { 
			...
			if ( actualTolerance ) { 
				try {
					variable_data.put(filter, dataQuery.getStreamFlowPeriodData(stations, whole_period.getStart(), whole_period.getEnd(), filter.getVariableType(), CalendarPeriod.WEEKLY));
				} catch ( InvalidArgumentException ive ) { 
					LOG.error("an invalid argument was used in the station filter", ive);
